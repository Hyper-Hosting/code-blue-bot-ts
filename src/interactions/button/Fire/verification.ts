  ...
  async Execute(interaction: ButtonInteraction) {
    ...
    if (userData.mainDepartment === "safr") {
      ...
      try {
        ...
        errMsg += "```Unable to add the Recruit role```\n";
      }

      try {
        await member.roles.add(safrServerData.DEPARTMENT_RANKS_ROLE_ID);
      } catch (error) {
        errMsg += "```Unable to add the Department Ranks role```\n";
      }
    } else {
