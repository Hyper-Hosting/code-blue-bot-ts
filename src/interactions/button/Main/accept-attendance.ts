...
export default class Inter extends Interaction {
  ...
  async Execute(interaction: ButtonInteraction) {
    ...
    if (member) {
      member.send({
        content: "Your attendance has been accepted",
      });
    }

    interaction.message.delete();
    interaction.reply({
      content: `Attendance Accepted`,
      flags: "Ephemeral",
    });
    ...
    if (staffUser) {
      await new LogsModel({
        ...
        timestamp: new Date(),
        logType: "Main",
        filterName: "Attendance Accept",
        logTitle: `${staffUser.firstName} ${staffUser.lastInitial} accepted ${userData.firstName} ${userData.lastInitial}'s attendance`,
        iconName: "check",
        info: [
